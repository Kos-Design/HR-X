...
void doposkselector() {
  char vbuttonslabels[numberofvbuttonslabels][12] = {"CuePlay", "Backward -1", "Stop", "Play", "Forward +1" ,"Record","Thingy", "Exit"};
  byte startyp = 8 ;
   byte starx = 1 ;
   byte ecart = 14 ;
   byte knobradius = 7 ;
  ...
  if ( sublevels[navlevelvbuttons] < numberofvbuttonslabels ) {
   ...
   if ( sublevels[navlevelvbuttons] >= numberofvbuttonslabels  && sublevels[navlevelvbuttons] < numberofvbuttonslabels +7 ) {
          ...
          canvasBIG.print(sublevels[navlevelvbuttons] + 1 - (numberofvbuttonslabels ));
    }
          canvasBIG.setCursor(97,0); 
          canvasBIG.print("CC") ;
   ...
   if ( sublevels[navlevelvbuttons] >= numberofvbuttonslabels +7  && sublevels[navlevelvbuttons] < numberofvbuttonslabels +14 ) {
    //Serial.println( "not a pot");
        canvasBIG.fillRoundRect(119, 16+((sublevels[navlevelvbuttons]-numberofvbuttonslabels-7)*7), 9, 6, 1, SSD1306_WHITE);
          if ( midiknobassigned[(70+sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels ))] != 0 ) {
    printassignedmidi(midiknobassigned[(70+sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels ))]);
      ...
      if ( midiknobassigned[ (sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels ))] != 0 ) {
    printassignedmidi(midiknobassigned[(70+sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels ))]);
    ...
    printassignedmidi(midiknobassigned[(70+sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels ))]);
      ...
      xcentershifter = 97-(((knobradius * 2 ) + 4 )*(sublevels[navlevelvbuttons] - numberofvbuttonslabels -14 - 6 - 5   ));
       canvasBIG.drawCircle(xcentershifter, 16+knobradius, knobradius-1, SSD1306_WHITE);
  ...
  if ( sublevels[navlevelvbuttons] < numberofvbuttonslabels +14 ) {
    ...
    if ( sublevels[navlevelvbuttons] < numberofvbuttonslabels  ) {
     ...
     //Serial.println(CClaval0);
      ...
      int CClaval = 70+(sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels )) ;
      ...
      //Vbuttonspanel();
       ...
       if (navlevel == navlevelvbuttons) {
         ...
         navrange = numberofvbuttonslabels-1 +17+14 ;
         sublevels[navlevelvbuttons + 1] = vPots[sublevels[navlevelvbuttons]-numberofvbuttonslabels-14] ;
          sublevels[navlevelvbuttons + 2] = vPots[sublevels[navlevelvbuttons]-numberofvbuttonslabels-14] ;
          } else {
          
    if (navlevel == navlevelvbuttons + 1) {     
     
      navrange = 127 ;
      vPots[sublevels[navlevelvbuttons]-numberofvbuttonslabels - 14] = sublevels[navlevelvbuttons + 1] ;
      
        if ( sublevels[navlevelvbuttons] > numberofvbuttonslabels +14 ) {
       int CClaval2 = 70+(sublevels[navlevelvbuttons] + 1 - ( numberofvbuttonslabels )) ;
      moncontrollercc( (byte)1,(byte)CClaval2, (byte)(vPots[sublevels[navlevelvbuttons]-numberofvbuttonslabels - 14]));
...
byte knobradius = 7 ;
 ...
 canvasBIG.drawRoundRect(119, 16+(i*7), 9, 6, 1, SSD1306_WHITE);
  ...
  //  float coeffangle = ((float)vPots[6+i]/128.0)*0.05 ;
    ...
    canvasBIG.drawCircle(centercirclex+knobradius+2, centercircley, knobradius, SSD1306_WHITE);
     display.drawLine(centercirclex+knobradius+2,centercircley,trianglepointx,trianglepointy,SSD1306_WHITE);
...
void drawtransport() {
 ...
 //canvasBIG.drawRoundRect(i*16, startyp-2, 16, 11, 2, SSD1306_WHITE);
  ...
  //cuestart
    ...
    canvasBIG.drawLine(starx+2, startyp+1, starx+2, startyp+5, SSD1306_WHITE);
    canvasBIG.print(">");
    //exit
