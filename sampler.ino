...
void setlefilenamed(int lefolder, int lefile, char* lefname) {
  ...
  int stringsize = (strlen((char*)samplefullpath[lefolder][lefile])) + fnamesize ;
...
void makenewdumpedfilename(int leflashfile ){
  ...
  newmksamplefullpath[strlen((char*)newmkdirpath)] = (char)'/' ;
    ...
    newmksamplefullpath[i+strlen((char*)newmkdirpath)+1] = Flashsamplename[leflashfile][i];
  }
...
void drawtickboxflashBIG(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor , int lefile){
...
void drawtickboxflashtitle(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor, int lefile) {
   if (Flashsamplesselected[lefile] == 1) {
...
void drawtickboxfolderBIG(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor , int lefolder){
...
void drawtickboxfoldertitle(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor, int lefolder) {
   if (samplesfoldersselected[lefolder]) {
    canvastitle.fillRect(lestartx,lestarty, lasizex, lasizey, lacolor);
  } else {
  canvastitle.drawRect(lestartx,lestarty, lasizex, lasizey, lacolor);
...
void drawtickboxincanvasBIG(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor ,int lefolder, int lefile){
...
void drawtickboxincanvastitle(int lestartx, int lestarty, int lasizex, int lasizey, typeof(SSD1306_WHITE) lacolor, int lefolder, int lefile) {
   if (samplesselected[lefolder][lefile] == 1) {
    canvastitle.fillRect(lestartx,lestarty, lasizex, lasizey, lacolor);
  } else {
  canvastitle.drawRect(lestartx,lestarty, lasizex, lasizey, lacolor);
...
void addtoFlashsamplelist(char* lesample) {
  ...
  for (int i = 0 ; i< 13 ; i++ ) {
     Flashsamplename[numberofFlashfiles][i] = toupper((unsigned char)(lesample[i]));
     if (i < (strlen((char*) lesample) - 4) ) {
      Flashsamplebase[numberofFlashfiles][i] = toupper((unsigned char)(lesample[i]));
     }
  }
  numberofFlashfiles++ ;
} 
...
if ( maxsizefirstpart > 6 ) {
  ...
  drawtickboxflashBIG(startx-13, starty+((filer)*10), 6, 6, SSD1306_WHITE, sublevels[navlevel]+1+filer);
 }
...
if ( maxsizelastpart > 6 ) {
  ...
  canvasBIG.setCursor(startx,(10*(numberofFlashfiles-sublevels[navlevel]))+6+((filer)*10));
  canvasBIG.println((char*)Flashsamplebase[filer]);
  drawtickboxflashBIG(startx-13, (10*(numberofFlashfiles-sublevels[navlevel]))+6+((filer)*10), 6, 6, SSD1306_WHITE, filer);
...
//Serial.println("listing samplerList");
  ...
  int starty = 16;
   ...
   canvasBIG.fillScreen(SSD1306_BLACK);
//Serial.println("doing other part");
int maxsizefirstpart = sizeofsamplefolder[sublevels[3]]-1-(sublevels[navlevel]);
if ( maxsizefirstpart > 6 ) {
  maxsizefirstpart = 6 ;
}
    for (int filer = 0 ; filer < maxsizefirstpart ; filer++) {
  canvasBIG.setCursor(startx,starty+((filer)*10));
  canvasBIG.println((char*)samplebase[sublevels[3]][sublevels[navlevel]+1+filer]);
  drawtickboxincanvasBIG(startx-13, starty+((filer)*10), 6, 6, SSD1306_WHITE, sublevels[3], sublevels[navlevel]+1+filer);
...
if ( maxsizelastpart > 6 ) {
  maxsizelastpart = 6 ;
    ...
    for (int filer = 0 ; filer < maxsizelastpart ; filer++) {
  canvasBIG.setCursor(startx,(10*(sizeofsamplefolder[sublevels[3]]-sublevels[navlevel]))+6+((filer)*10));
  canvasBIG.println((char*)samplebase[sublevels[3]][filer]);
  drawtickboxincanvasBIG(startx-13, (10*(sizeofsamplefolder[sublevels[3]]-sublevels[navlevel]))+6+((filer)*10), 6, 6, SSD1306_WHITE, sublevels[3], filer);
...
if ( maxsizefirstpart > 6 ) {
  maxsizefirstpart = 6 ;
    ...
    for (int filer = 0 ; filer < maxsizefirstpart ; filer++) {
  canvasBIG.setCursor(startx,starty+((filer)*10));
  canvasBIG.println((char*)samplefoldersregistered[sublevels[navlevel]+1+filer]);
  if ( sublevels[2]==1) {
    //if ( !(isitadir((char*)samplefoldersregistered[sublevels[navlevel]+1+filer]))) {
   drawtickboxfolderBIG(startx-13, starty+((filer)*10), 6, 6, SSD1306_WHITE, sublevels[navlevel]+1+filer);
   // }
  }
  }
  //Serial.println("last part");
...
if ( maxsizelastpart > 6 ) {
  maxsizelastpart = 6 ;
    ...
    for (int filer = 0 ; filer < maxsizelastpart ; filer++) {
  canvasBIG.setCursor(startx,(10*(sampledirsregistered-sublevels[navlevel]))+6+((filer)*10));
  canvasBIG.println((char*)samplefoldersregistered[filer]);
   //checkbox
   if ( sublevels[2]==1) {
  
   drawtickboxfolderBIG(startx-13, (10*(keepcount-sublevels[navlevel]))+6+((filer)*10), 6, 6, SSD1306_WHITE, filer);
  //  }
    }
  }
...
void dolistLoadSampleMenu() {
  ...
  char menuloadsample[sizeofmenuloadsample][16] = {"Select","Whole pack","Load Selection","Clear Selection"};
    int startx = 5;
  int starty = 16;
...
// canvastitle.println("%");
    ...
    for (int filer = 0 ; filer < sizeofmenuloadsample-1 -(sublevels[2]) ; filer ++) {
...
canvasBIG.setCursor(startx,(10*(sizeofmenuloadsample-sublevels[2])+6+((filer)*10)));
...
char menudelsample[sizeofmenudelsample][16] = {"Select","Remove All","Remove Selected","Clear Selection"};
    int startx = 5;
  int starty = 16;
 ...
 //Serial.println(textin);
    ...
    for (int filer = 0 ; filer < sizeofmenudelsample-1 -(sublevels[2]) ; filer ++) {
...
canvasBIG.setCursor(startx,(10*(sizeofmenudelsample-sublevels[2])+6+((filer)*10)));
...
char menuassignsample[sizeofmenuassignsample][19] = {"Auto-assign","Individual","Clear All","Save assigned"};
 ...
 //Serial.println(textin);
    ...
    for (int filer = 0 ; filer < sizeofmenuassignsample-1 -(sublevels[2]) ; filer ++) {
    ...
    for (int filer = 0 ; filer < sublevels[2] ; filer ++) {
      ...
      canvasBIG.setCursor(startx,(10*(sizeofmenuassignsample-sublevels[2])+6+((filer)*10)));
...
char samplerlabels[sizeofsamplerlabels][12] = {"Load", "Delete", "Assign","Recorder","Mixer"};
    int startx = 5;
  int starty = 16;
...
// Serial.println(textin);
    ...
    for (int filer = 0 ; filer < sizeofsamplerlabels-1 -(sublevels[1]) ; filer ++) {
...
canvasBIG.setCursor(startx,(10*(sizeofsamplerlabels-sublevels[1])+6+((filer)*10)));
...
void doConfirmpanel(char* letitlemsg) {
 ...
 //canvasBIG.print((char*)text);
    ...
    canvasBIG.drawRect(14+sublevels[navlevel]*66, 23, 23+sublevels[navlevel]*6, 17, SSD1306_WHITE);
  ...
  for ( int i = 0 ; i < 99; i++) {
    ...
    lengthz = currentsample.size();
     ...
     currentFlashfile = SerialFlash.open(currentflashname);
      if (currentFlashfile && currentFlashfile.size() == currentsample.size()) {
...
void samplesetter() {
  Sampleassigned[sublevels[3]] = sublevels[4];
}
