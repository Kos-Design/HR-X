...
//printnoteon(channel, data1, data2);
    ...
    if (!digitalplay) {
      //if ( lesampleliner < 4 ) {
    Flashmixer[int(lesampleliner/4)]->gain(lesampleliner-4*int(lesampleliner/4), (smixervknobs[lesampleliner]/128.0)*(data2/128.0));
...
//      }
      ...
      Flashmixer[int(lesampleliner/4)]->gain(lesampleliner-4*int(lesampleliner/4), (smixervknobs[lesampleliner]/128.0));
    }
...
if (debugmidion) {debugmidi((char*)"ProgramChange",(int)( channel),(int)( data1));}
...
void MaNoteOff(byte channel, byte data1, byte data2) {
  ...
  int lachordnote ;
   //if (debugmidion) {debugmidi("NoteOff",(int)( channel),(int)( data1),(int)( data2));}
   Serial.print("Off, ch=");
...
//void recordsynthduration( int liner ,byte channel , byte lenote, byte velocity){
...
void clearbouncedoffsafters(int linei, byte lanotef, int lapos) {
  ...
  if ( (laposoftheNofafter < pbars) && (laposoftheNofafter != 0 )){
    ...
    bool resultNoteOninbetween = testforaNoteOninbetween(linei, lapos, laposoftheNofafter,lanotef);
    if (resultNoteOninbetween) {
     ...
     // Serial.println((int)resultNoteOninbetween);
      ...
      if ( (lapos == 0 ) && ((event1notesOff[linei][pbars-1][1] == lanotef) && event1notes1[linei][pbars-1][1] != lanotef )) {
 ...
 int lewhilecountmax = 0 ;
  ...
  latargetpos = (lesposes[0][lesnons-1]) ;
    ...
    //  Serial.println((int)event1notes1[linei][latargetpos][1]);
         event1notesOff[linei][latargetpos+1][0] = event1notes1[linei][latargetpos][0] ;
          event1notesOff[linei][latargetpos+1][1] = event1notes1[linei][latargetpos][1] ;
          event1notesOff[linei][0][0] = (byte)0 ;
          event1notesOff[linei][0][1] = (byte)0 ;
        //addone then at +1
       } 
...
//        
//         leresulte = anoteOffisnext(linei, event1notes1[linei][lesposes[0][i]][1], lesposes[0][i]) ;
...
void computelenghtmesureoffline(){ 
    for (int linei = 0 ; linei < nombreofliners ; linei++ ) {
       for (int i = 0; i < pbars ; i++ ) {
        ...
        int laposof = getnextposofevent1Off(linei,event1notes1[linei][i][1], i);
...
void continueCCinterpol(byte lecc) {
 ...
 laCCduration =  Ccinterpolengh[lecc][2] * millitickinterval;
    ...
    if ( (Ccinterpolengh[lecc][1] - Ccinterpolengh[lecc][0]) > 0 ) {
      interpolcoeff = ( (((tickposition - Ccinterpolengh[lecc][0])* millitickinterval)+(millis() - tickerlasttick)+millitickinterval )  / (laCCduration*1.0)) ;
      letempipolate = event1controllers[lecc][Ccinterpolengh[lecc][0]] + ( interpolcoeff * ( event1controllers[lecc][Ccinterpolengh[lecc][1]] - event1controllers[lecc][Ccinterpolengh[lecc][0]] ) ) ;
    } 
   ...
   if (tickposition > Ccinterpolengh[lecc][0]) {
      ...
      interpolcoeff = ( (((tickposition - Ccinterpolengh[lecc][0])* millitickinterval)+(millis() - tickerlasttick) )  / (laCCduration*1.0)) ;
      
      letempipolate = event1controllers[lecc][Ccinterpolengh[lecc][0]] + ( 1.0*interpolcoeff * ( event1controllers[lecc][Ccinterpolengh[lecc][1]] - event1controllers[lecc][Ccinterpolengh[lecc][0]] ) ) ;
     
      
      // leccinterpolated[i] = round(event1controllers[lecc][tickposition] + (((millis() - tickerlasttick)/(laCCduration*1.0))*  (event1controllers[lecc][tickposition+1] - event1controllers[lecc][tickposition] ))) ;
     } else {
       interpolcoeff =  ((( tickposition + pbars - Ccinterpolengh[lecc][0] )* millitickinterval)+(millis() - tickerlasttick) )  / (laCCduration*1.0) ;
      ...
      letempipolate = event1controllers[lecc][Ccinterpolengh[lecc][0]] + ( 1.0*interpolcoeff * ( event1controllers[lecc][Ccinterpolengh[lecc][1]] - event1controllers[lecc][Ccinterpolengh[lecc][0]] ) ) ;
        ...
        interpolcoeff =  ( ( millis() - tickerlasttick)/(laCCduration*1.0) )  ;
          letempipolate = event1controllers[lecc][tickposition] + ( 1.0*interpolcoeff * ( event1controllers[lecc][Ccinterpolengh[lecc][1]] - event1controllers[lecc][tickposition] ) ) ;
...
//     Serial.println(tickposition);
     if ( leccinterpolated[lecc] == event1controllers[lecc][Ccinterpolengh[lecc][1]] ) {
      activateinterpolatecc[lecc] = 0;
      Ccinterpolengh[lecc][0] = pbars ;
      Ccinterpolengh[lecc][1] = pbars ;
      Ccinterpolengh[lecc][2] = 0 ;
