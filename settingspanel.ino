  ...
  byte startlex = 4 ;
   ...
   byte ecartl = 19 ;
    ...
    int slct = sublevels[2];
      canvasBIG.drawLine(startlex-2+slct*ecartl, topwbarstart+totbartall+2, startlex-2+slct*ecartl, topwbarstart+totbartall+1+wbarwidth2+3-1 , SSD1306_WHITE ) ;
      canvasBIG.drawLine(startlex-2+1+slct*ecartl, topwbarstart+totbartall+wbarwidth2+4, wbarwidth2+3+startlex-4+slct*ecartl, topwbarstart+totbartall+wbarwidth2+4, SSD1306_WHITE ) ;
...
void arpegiatorVpanel() {
  ...
  byte slct = sublevels[2] ;
     byte wbarwidth2 = 7 ;
   char lesarpegestype[8][12] = {"Ionian", "Dorian", "Phrygian", "Lydian", "Mixolydian", "Aeolian", "Harmonic", "Locrian"  } ;
    display.clearDisplay();
    canvasBIG.fillScreen(SSD1306_BLACK);
    canvastitle.fillScreen(SSD1306_BLACK);
    canvastitle.setCursor(0,0);
    ...
    byte arpegiovalues[7]={arpegiatortype,arpegmode,arpegstartoffset,arpegnumofnotes,arpeggridC,arpeggridS,arpeglengh};
    float lesarparams[7] = {letype,lemode,leoffset,lenumofnotes,legridC,legridS,lelengh} ;
    char lesbarlabels[7][12] = {"Type","Mode","Offset","Arp. size","Notes Block","Empty space","Length"};
    
    char lesarpegesmodes[8][12] = {"Up forward", "Down back", "Up down f.", "Down up f.", "Mozart", "Down up b.", "Up down b.", "Mandalorian"  } ;
...
for (int i = 0 ; i < 7 ; i++ ) {
    //barsize = round(((WetMixMasters[lefilter])*(totbartall-4))) ;
     canvasBIG.drawRoundRect(startlex+i*ecartl, topwbarstart, wbarwidth2, totbartall ,2, SSD1306_WHITE ) ;
      canvasBIG.fillRect(startlex+i*ecartl, topwbarstart+2+((totbartall-7)-(lesarparams[i]*(totbartall-7))) , wbarwidth2 , 3,  SSD1306_WHITE ) ;
      canvasBIG.setCursor(startlex+1+i*ecartl,topwbarstart+totbartall+2);
...
if (arpegiatortype !=8){
  ...
  if (sublevels[1] == 13 ) {
    ...
    for (int filer = 0 ; filer < numbofsettinglabels-1 -(sublevels[1]) ; filer++) {
      ...
      canvasBIG.setCursor(startx,(10*(numbofsettinglabels-sublevels[1])+6+((filer)*10)));
