...
class SettingsMenuRouter : public SectionHolder {
    public:
        ...
        void attach_settings_menus(uint8_t index, void (*cb)())
        {
            if (index < numbofsettinglabels)
                _settings_menu[index] = cb;
        }
        void settings_nav_one(){
          canvasBIG.setTextSize(1);
          canvastitle.setTextSize(1);
          switch (sublevels[1]) {
    ...
    private:
        void (*_settings_menu[numbofsettinglabels])() = {nullptr};
};

SettingsMenuRouter _st = SettingsMenuRouter();