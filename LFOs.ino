...
void LFOrmType(int leLFO) {
  if (navlevel == 3 ) {
   ...
   switch( LFOformstype[leLFO]  ) {
    case 0 :
    displayLFOrmimg(sublevels[3],(char*)"SineWave",sinewave, leLFO, WAVEFORM_SINE);
    break;
    case 1 :
    displayLFOrmimg(sublevels[3],(char*)"SawWave",sawtoothwave,leLFO,WAVEFORM_SAWTOOTH);
    break;
    case 2 :
    displayLFOrmimg(sublevels[3],(char*)"ReverseSaw",reversesawtoothwave,leLFO,WAVEFORM_SAWTOOTH_REVERSE);
    break;
    case 3 :
    displayLFOrmimg(sublevels[3],(char*)"Triangle",trianglewave,leLFO,WAVEFORM_TRIANGLE);
    break;
    case 4 :
    displayLFOrmimg(sublevels[3],(char*)"V-Triangle",variabletriangle,leLFO,WAVEFORM_TRIANGLE_VARIABLE);
    break;
    case 5 :
    displayLFOrmimg(sublevels[3],(char*)"SquareWave",squarewave,leLFO,WAVEFORM_SQUARE);
    break;
    case 6 :
    displayLFOrmimg(sublevels[3],(char*)"PulseWave",pulsewave,leLFO,WAVEFORM_PULSE);
    break;
    case 7 :
    displayLFOrmimg(sublevels[3],(char*)"Arbitrary",arbitrarywave,leLFO,WAVEFORM_ARBITRARY);
    break;
    case 8 :
    displayLFOrmimg(sublevels[3],(char*)"SampleHold",samplehold,leLFO,WAVEFORM_SAMPLE_HOLD);
    break;
...
void restartLFO( int leLFO) {
  ...
  LFOwaveforms1[leLFO]-> begin((float)(LFOlevel[leLFO]/512.00), ((LFOfreqs[leLFO]/100.0)*(1000/millitickinterval)), lesformes[LFOformstype[leLFO]]);
  } else {
    LFOwaveforms1[leLFO]-> begin((float)(LFOlevel[leLFO]/512.00), (LFOfreqs[leLFO]/100.0)*0.5, lesformes[LFOformstype[leLFO]]);
    // lfosinez[leLFO]->frequency((LFOfreqs[leLFO]/100.0)*0.5);
  }
  if (leLFO == 3 ){
    LFOrm303.begin((float)(LFOlevel[leLFO]/512.00), ((LFOfreqs[leLFO]/100.0)*(1000/millitickinterval)), lesformes[LFOformstype[leLFO]]);
  }
     ...
     if (navlevel == 3 ) {
       // Serial.println("Setting LFO level");
        navrange = 512 ;
         ...
         //restartLFO(leLFO);
        } else { sublevels[3]= LFOlevel[leLFO]; }
      if (navlevel >= 4 ) {
      gobacktolfoparams();  
      }
...
void doLFOoffset(int leLFO) {
   ...
   doLFOallcontrols(leLFO);
           //restartLFO(leLFO);
  } else { sublevels[3]= LFOoffset[leLFO]; }
      if (navlevel >= 4 ) {
     gobacktolfoparams();
  }
...
void doLFOphase(int leLFO) {
     if (navlevel == 3 ) {
       // Serial.println("Setting LFO phase");
        ...
        doLFOallcontrols(leLFO);
          //restartLFO(leLFO);
  } else { sublevels[3]= LFOphase[leLFO]; }
      if (navlevel >= 4 ) {
       gobacktolfoparams();
  }
...
void doLFOfreqd(int leLFO) {
  //set lfosine1 lfosinez[leLFO]->
     if (navlevel == 3 ) {
        ...
        //doLFOallcontrols(leLFO);
  } else { sublevels[3]= LFOfreqs[leLFO]; }
      if (navlevel >= 4 ) {
       gobacktolfoparams();
     }
...
void dolistLFOparams() {
   char LFOlabels[sizeofLFOlabels][12] = {"Type", "Level", "Sync", "Freq", "Offset", "Phase", "Synth", "<-  ","  ->"};
   display.clearDisplay();
    int startx = 5;
  int starty = 16;
   char* textin = (char*)LFOlabels[sublevels[2]] ;
    ...
    for (int filer = 0 ; filer < sublevels[2]  ; filer++) {
        ...
        canvasBIG.setCursor(startx,(10*(sizeofLFOlabels-sublevels[2])+6+((filer)*10)));
