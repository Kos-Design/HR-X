...
void switchfxes(byte lafxline, byte leeffect) {
 ...
 AudioNoInterrupts();
     ...
     // if effect on liner is different or same, is a slot of the 3 modules of the leeffect used here -> clean and do
     // if le effect has one slot used but not used here ignore and let getavailable do its job
...
void dolistMainFxPanel() {
  char mainmenufxlist[mainmenufxlistsize][12] = {"Multiply", "Reverb", "Granular", "BitCrusher","Flanger", "Chorus", "Biquad", "Filter", "Delay", "None"};
    byte startx = 5;
  byte starty = 16;
  ...
  if (moduleonfxline[sublevels[1]][0] != (mainmenufxlistsize-1)) {
    ...
    for (int filer = 0 ; filer < mainfxlines-1 -(sublevels[1]) ; filer++) {
     ...
     canvasBIG.setCursor(startx,starty+((filer)*10));
      ...
      if (moduleonfxline[sublevels[1]+1+filer][0] != (mainmenufxlistsize-1)) {
        canvasBIG.println((char*)mainmenufxlist[moduleonfxline[sublevels[1]+1+filer][0]]);
     //textin = (char*)mainmenufxlist[moduleonfxline[sublevels[1]][0]];
        } else {
    ...
    for (int filer = 0 ; filer < sublevels[1]  ; filer++) {
      ...
      canvasBIG.setCursor(startx,(10*(mainfxlines-sublevels[1])+6+((filer)*10)));
...
switch (bqtype[lebiquad][bqstage[lebiquad]] ) {
  case 0 :
    ...
    biquad[lebiquad]->setLowpass(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]] );
    
    break;
   case 1 :
   
   biquad[lebiquad]->setHighpass(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]] );
    
  break;
   case 2 :
   
   biquad[lebiquad]->setBandpass(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]] );
   
  break;
   case 3 :
  
   biquad[lebiquad]->setNotch(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]] );
   
  break;
   case 4 :
   
   biquad[lebiquad]->setLowShelf(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqgain[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]]);
   
  break;
   case 5 :
   
   biquad[lebiquad]->setHighShelf(bqstage[lebiquad],bqfreq[lebiquad][bqstage[lebiquad]],bqgain[lebiquad][bqstage[lebiquad]],bqslope[lebiquad][bqstage[lebiquad]]);
...
void flangercontrols(byte lefilter) {
  flangeoffset = round((flangerVknobs[lefilter][0]/128.0)*FLANGE_DELAY_LENGTH/4 ); 
  flangedepth = round((flangerVknobs[lefilter][1]/128.0)*FLANGE_DELAY_LENGTH/4) ;
...
void flangerVpanel(byte lefilter) {
  ...
  //  flangerVknobs[lefilter][0] ;
   ...
   // float freqflange = flangerVknobs[lefilter][2]/128.0 ;
    ...
    //Roomsize
    coeffangle = (6.2831-(flangerVknobs[lefilter][0]/128.0)*6.2831)+3.1416 ;
    ...
    canvasBIG.print(round((flangerVknobs[lefilter][0]/128.0)*FLANGE_DELAY_LENGTH/4));
    ...
    canvasBIG.print("Offset");
     ...
     //damping
    coeffangle = (6.2831-(flangerVknobs[lefilter][1]/128.0)*6.2831)+3.1416 ;
    ...
    canvasBIG.setCursor(centercirclex-knobradius+1, centercircley+ knobradius + 4);
    canvasBIG.setTextSize(1);
     canvasBIG.print(round((flangerVknobs[lefilter][1]/128.0)*FLANGE_DELAY_LENGTH/4));
    //canvasBIG.setCursor(centercirclex-knobradius+1, 8);
    canvasBIG.setCursor(centercirclex-knobradius-2, centercircley+ knobradius + 4 +10);
    canvasBIG.print("Depth");

       coeffangle = (6.2831-(flangerVknobs[lefilter][2]/128.0)*6.2831)+3.1416 ;
    centercirclex = centercirclex+40 ;
    canvasBIG.drawCircle(centercirclex, centercircley, knobradius, SSD1306_WHITE);
    ftVcursorpointx = round(centercirclex + (knobradius*( cos(coeffangle) )));
    ftVcursorpointy = round(centercircley - (knobradius*( sin(coeffangle) )));
...
void bitcrusherctrl(byte lefilter) {
  bitcrusher[lefilter]->bits(bitcrusherVknobs[lefilter][0]);
   bitcrusher[lefilter]->sampleRate(round((bitcrusherVknobs[lefilter][1]/128.0)*44100));
}
...
if ( LFOonfilterz[lefilter] < 3 ) {
   LFOtoFilterz[((fxiterations*lefilter) + LFOonfilterz[lefilter])]->connect();
 }
}
...
//    canvastitle.print((char*)bqtypeLabels[bqtype[lefilter][bqstage[lefilter]]]);
    ...
    //bqfreq
    coeffangle = (6.2831-(ffilterzVknobs[lefilter][0]/128.0)*6.2831)+3.1416 ;
    canvasBIG.drawCircle(centercirclex, centercircley, knobradius, SSD1306_WHITE);
    ftVcursorpointx = round(centercirclex + (knobradius*( cos(coeffangle) )));
    ftVcursorpointy = round(centercircley - (knobradius*( sin(coeffangle) )));
    canvasBIG.drawLine(centercirclex,centercircley,ftVcursorpointx,ftVcursorpointy,SSD1306_WHITE);
    ...
    canvasBIG.setCursor(centercirclex-knobradius-5, 24);
     ...
     //resonnance
    coeffangle = (6.2831-(ffilterzVknobs[lefilter][1]/128.0)*6.2831)+3.1416 ;
    ...
    //octave
    coeffangle = (6.2831-(ffilterzVknobs[lefilter][2]/128.0)*6.2831)+3.1416 ;
...
//stage
  ...
  if (slct == 2) {
   ...
   bqfreq[lebiquad][bqstage[lebiquad]]= (((bqVpot[lebiquad][bqstage[lebiquad]][0])/128.0) * bqrange )+ 101;
  }
  //slope or Q
  if (slct == 3) {
    navrange = 128 ;
    bqVpot[lebiquad][bqstage[lebiquad]][1] = sublevels[4] ;
    bqslope[lebiquad][bqstage[lebiquad]] = (bqVpot[lebiquad][bqstage[lebiquad]][1])/128.0 ;
 ...
 if (slct == 5) {
  navrange = 128 ;
     bqVpot[lebiquad][bqstage[lebiquad]][2] = sublevels[4] ;
      bqgain[lebiquad][bqstage[lebiquad]]= (bqVpot[lebiquad][bqstage[lebiquad]][2])/128.0 ;
  }
  //to avoid setting up a stage unconfigured while browsing  
    ...
    if (slct == 4) {
  navrange = 100 ;
     WetMixMasters[(fxmoduleisconnected[sublevels[2]][lebiquad]+1)] = sublevels[4]/100.0 ;
  wetmixmastercontrols();
    //le303filterzwet = (mixle303ffilterzVknobs[2])/128.0 ;
    //applygain
   //le303filterzWet();
...
void biquadVpanel(byte lebiquad) {
  char bqtypeLabels[6][12] = {"Low Pass", "High Pass", "Band Pass", "Notch", "LowShelf", "High Shelf" };
  ...
  byte centercircley = 28 + knobradius ;
   ...
   byte bqVcursorpointy ; 
    ...
    byte wbarwidth = 9 ;
    byte barsize = round((bqVpot[lebiquad][bqstage[lebiquad]][2]/128.0)*(totbartall-4)) ;
    ...
    //bqfreq
    coeffangle = (6.2831-(bqVpot[lebiquad][bqstage[lebiquad]][0]/128.0)*6.2831)+3.1416 ;
    canvasBIG.drawCircle(centercirclex, centercircley, knobradius, SSD1306_WHITE);
    ...
    canvasBIG.print(bqfreq[lebiquad][bqstage[lebiquad]],0);
     //bqslope
    coeffangle = (6.2831-(bqVpot[lebiquad][bqstage[lebiquad]][1]/128.0)*6.2831)+3.1416 ;
   ...
   canvasBIG.setCursor(90,18);
    canvasBIG.print(bqgain[lebiquad][bqstage[lebiquad]]);
      ...
      barsize = round(((WetMixMasters[(fxmoduleisconnected[sublevels[2]][lebiquad]+1)])*(totbartall+13-4))) ;
...
//stage
 ...
 if (slct == 5) {
  ...
  if (slct == 4) {
  sublevels[4] = round(WetMixMasters[(fxmoduleisconnected[sublevels[2]][lebiquad]+1)]*100.0);
  canvasBIG.drawRect(topwbarstart+startlex2+4, 0+2  , totbartall , wbarwidth2-4, SSD1306_WHITE ) ;
  canvasBIG.fillRect(55,2,3,3,SSD1306_WHITE);
}
}
