...
void dolistpresetsmenu() {
  char presetmenulabels[truesizeofpresetmenulabels][12] = {"Save", "Load", "Copy", "Delete", "Params"};
    byte startx = 5;
  byte starty = 16;
 ...
 //Serial.println(textin);
    ...
    for (int filer = 0 ; filer < truesizeofpresetmenulabels-1 -(sublevels[1]) ; filer ++) {
    ...
    for (int filer = 0 ; filer < sublevels[1]  ; filer ++) {
      ...
      canvasBIG.setCursor(startx,(10*(truesizeofpresetmenulabels-sublevels[1])+6+((filer)*10)));
...
void dolistofpresets() {
  ...
  // char* textinz = (char*)lesfiles[sublevels[3]].name() ;
    ...
    canvasBIG.fillScreen(SSD1306_BLACK);
     if ( sublevels[2] == numberofSynthPresets ) {
      for (int filer = 0 ; filer < sublevels[2]-1  ; filer ++) {
        canvasBIG.setCursor(startx,(10*(numberofSynthPresets-sublevels[2]))+16+((filer)*10));
        canvasBIG.println((char*)SynthPresetbase[filer]);
      }
        } else {
        for (int filer = 0 ; filer < numberofSynthPresets-1-(sublevels[2]) ; filer ++) {
      canvasBIG.setCursor(startx,starty+((filer)*10));
    ...
    for (int filer = 0 ; filer < sublevels[2]  ; filer ++) {
        ...
        canvasBIG.setCursor(startx,(10*(numberofSynthPresets-sublevels[2]))+6+((filer)*10));
        canvasBIG.println((char*)SynthPresetbase[filer]);
       }
     } 
